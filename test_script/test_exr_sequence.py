...
def read_exr_rgb(path):
    if hasattr(OpenEXR, "InputFile"):
        try:
            return _read_exr_legacy(path)
        except Exception:
            if hasattr(OpenEXR, "File"):
                return _read_exr_modern(path)
            raise
    return _read_exr_modern(path)


def save_exr(output_path, depth):
...
def _save_depth_worker(args):
    ...
    save_exr(output_path, depth_frame)
    return output_path
